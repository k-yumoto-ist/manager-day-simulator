...
function App() {
  ...
  if(phase==='intro') return <div className={`mode-theme mode-${selectedMode}`}><StartScreen onStart={startGame} onRules={()=>setPhase('rules')} selectedMode={selectedMode} onSelectMode={setSelectedMode}/></div>
  if(phase==='rules') return <div className={`mode-theme mode-${selectedMode}`}><RulesScreen onBack={()=>setPhase('intro')} onStart={startGame}/></div>
  if(phase==='result') return <div className={`mode-theme mode-${selectedMode}`}><ResultScreen time={time} metrics={metrics} tasks={tasks} stats={stats} unread={unreadCount} logs={logs} interactions={interactions} events={visible} meetingAttention={meetingAttention} delegates={delegateList} mode={modeData.config} onRestart={restart}/></div>
  if(isMobile) return <div className={`mode-theme mode-${selectedMode}`}><MobileGameLayout time={time} metrics={metrics} totalTask={totalTask} activeMeeting={activeMeeting} meetingAttention={meetingAttention} unreadCount={unreadCount} mentionCount={mentionThreads.length} threads={threads} filteredThreads={filteredThreads} selected={selected} selectedLast={selectedLast} selectedTimeline={selectedTimeline} reactions={reactions} snoozed={snoozed} read={read} inboxFilter={inboxFilter} activeMeetings={activeMeetings} meetingChoices={meetingChoices} meetingEnds={meetingEnds} tasks={tasks} toast={toast} meetingPrompt={meetingPrompt} reactionFor={reactionFor} delegateFor={delegateFor} delegates={delegateList} endConfirm={endConfirm} unfinishedTasks={unfinishedTasks} pendingCount={pendingCount} mobileTab={mobileTab} mobileDetail={mobileDetail} mobileMore={mobileMore} onTab={setMobileTab} onOpenThread={(id,events)=>{chooseThread(id,events);setMobileDetail(true);setMobileMore(false)}} onBack={()=>setMobileDetail(false)} onFilter={setInboxFilter} onAction={performAction} onReaction={applyReaction} onCloseReaction={()=>setReactionFor(null)} onDelegate={doDelegate} onCloseDelegate={()=>setDelegateFor(null)} onMeeting={(m,c)=>decideMeeting(m,c)} onFocus={focusTask} onEnd={endGame} onCancelEnd={()=>setEndConfirm(false)} onConfirmEnd={confirmEndGame} onToggleMore={()=>setMobileMore(x=>!x)} />{missedMeeting&&<MobileMissedMeetingPrompt meeting={missedMeeting} onResolve={resolveMissed}/>}</div>

  return <div className={`app-shell mode-theme mode-${selectedMode}`}>
...
function MobileMissedMeetingPrompt({meeting,onResolve}:{meeting:Meeting;onResolve:(choice:'yes'|'repeat'|'member')=>void}){
  const dialogRef=useRef<HTMLDivElement>(null)
  useEffect(()=>{const previous=document.activeElement as HTMLElement|null;const dialog=dialogRef.current;const buttons=dialog?[...dialog.querySelectorAll<HTMLButtonElement>('button')]:[];buttons[0]?.focus();const trap=(event:KeyboardEvent)=>{if(event.key!=='Tab'||buttons.length===0)return;const first=buttons[0],last=buttons[buttons.length-1];if(event.shiftKey&&document.activeElement===first){event.preventDefault();last.focus()}else if(!event.shiftKey&&document.activeElement===last){event.preventDefault();first.focus()}};document.addEventListener('keydown',trap);return()=>{document.removeEventListener('keydown',trap);previous?.focus()}},[])
  return <div ref={dialogRef} className="mobile-sheet-backdrop mobile-missed-backdrop" role="alertdialog" aria-modal="true" aria-labelledby="mobile-missed-title"><div className="mobile-sheet mobile-missed-sheet"><span className="eyebrow">MEETING ATTENTION</span><h2 id="mobile-missed-title">会議の内容を聞き逃しました</h2><p><strong>{meeting.title}</strong>の途中でチャット対応に注意が移り、依頼内容を十分に把握できていません。どう対応しますか？</p><div className="mobile-missed-options"><button onClick={()=>onResolve('yes')}><strong>承知しました</strong><small>内容が曖昧なまま進める</small></button><button onClick={()=>onResolve('repeat')}><strong>もう一度説明してもらう</strong><small>8分・認識を合わせ直す</small></button><button onClick={()=>onResolve('member')}><strong>後でメンバーに確認</strong><small>3分・相手には聞き返さない</small></button></div></div></div>
}
...
function ModePicker({selectedMode,onSelect}:{selectedMode:GameModeId;onSelect:(mode:GameModeId)=>void}){
  return <div className="mode-picker" aria-label="プレイモードを選択">{(Object.values(modeSettings)).map(mode=><button key={mode.id} className={`mode-card mode-card-${mode.id} ${selectedMode===mode.id?'selected':''}`} aria-pressed={selectedMode===mode.id} onClick={()=>onSelect(mode.id)}><span>{mode.id==='standard'&&<em>おすすめ</em>}{mode.estimatedPlayTime}</span><strong>{mode.label}</strong><small>{mode.description}</small></button>)}</div>
}
